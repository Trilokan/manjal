odoo.define('web.win', function (require) {
"use strict";

var config = require('web.config');
var core = require('web.core');
var _t = core._t;
var QWeb = core.qweb;
var FormRenderingEngine = require('web.FormRenderingEngine');


FormRenderingEngine.include({
  process_win: function($sheet) {
        var $new_sheet = this.render_element('FormRenderingwin', $sheet.getAttributes());
        this.handle_common_properties($new_sheet, $sheet);
        var $dst = $new_sheet.find('.o_form_win');
        $sheet.contents().appendTo($dst);
        $sheet.before($new_sheet).remove();
        this.process($new_sheet);
    },
});

});